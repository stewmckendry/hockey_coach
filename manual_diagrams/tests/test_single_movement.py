#!/usr/bin/env python3
"""Test single movement for coordinate accuracy."""
...
def analyze_movement_accuracy(spec, query):
    ...
    for idx, movement in enumerate(movements, 1):
        ...
        if movement['type'] in ['pass', 'shot']:
            if dx > 0:  # Moving toward offensive net (x=89)
                ...
                print(f"    ⚠️  Moving away from offensive zone")
                hockey_sense_issues.append(f"Movement {idx} goes backward")
        ...
        if movement['type'] == 'shot':
            ...
            if to_net_dist < 5:
                ...
                hockey_sense_issues.append(f"Shot {idx} misses net by {to_net_dist:.1f}")
        ...
        for pid, info in player_map.items():
            ...
            if start_dist < 5:
                start_near_player = (info['position'], start_dist)
            
            # Check end position
            end_dist = ((movement['to_pos']['x'] - info['coords']['x'])**2 + 
                       (movement['to_pos']['y'] - info['coords']['y'])**2)**0.5
            if end_dist < 5:
                end_near_player = (info['position'], end_dist)
        
        if start_near_player:
            print(f"    ✅ Starts near {start_near_player[0]} (dist: {start_near_player[1]:.1f})")
        else:
            print(f"    ⚠️  Doesn't start near any player")
            hockey_sense_issues.append(f"Movement {idx} doesn't start from a player")
            
        if movement['type'] == 'pass' and end_near_player:
            print(f"    ✅ Pass targets {end_near_player[0]} (dist: {end_near_player[1]:.1f})")
        elif movement['type'] == 'pass':
            print(f"    ⚠️  Pass doesn't clearly target a player")
            hockey_sense_issues.append(f"Pass {idx} doesn't target a player")
    ...
    if not hockey_sense_issues:
        print("✅ ALL MOVEMENTS MAKE HOCKEY SENSE!")
    else:
        print(f"❌ {len(hockey_sense_issues)} ISSUES FOUND:")
        for issue in hockey_sense_issues:
            print(f"  - {issue}")
    
    return len(hockey_sense_issues) == 0
...
def main():
    ...
    makes_sense = analyze_movement_accuracy(spec, scenario)
    
    # Save spec for inspection
    output_file = Path(__file__).parent / "outputs" / "single_movement_test.json"
    ...
    if makes_sense:
        print("\n🎉 SUCCESS: Movement coordinates make hockey sense!")
    else:
        print("\n⚠️  WARNING: Some movements don't make hockey sense")
