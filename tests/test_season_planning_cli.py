...
class SeasonPlanningCLI:
    ...
    async def _conversation_loop(self):
        ...
        while True:
            try:
                ...
                try:
                    # Use consistent session_id for conversation persistence
    ...
    async def _show_session_info(self):
        ...
        print(f"  Session Persistence: SQLiteSession (automatic)")
        print(f"  Database File: season_planning_conversations.db")
        
        # Try to get session information from agent
        try:
            if hasattr(self.agent, 'get_session_info'):
                session_info = self.agent.get_session_info(self.session_id)
                print(f"  Session Status: {session_info.get('status', 'active')}")
            else:
                print(f"  Session Status: Active (conversation history managed by SDK)")
        except Exception as e:
    ...
    for i, scenario in enumerate(test_scenarios, 1):
        ...
        try:
            ...
            for j, message in enumerate(scenario['messages'], 1):
                ...
                # Pass session_id for conversation persistence within each scenario
                response = await run_season_planning_agent(message, group_id=session_id, session_id=session_id)
