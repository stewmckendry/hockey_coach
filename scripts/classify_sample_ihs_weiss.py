...
JSON_OUT_PATH = SCRIPT_DIR.parent / "outputs" / "drills_classified_sample.json"
CSV_OUT_PATH = SCRIPT_DIR.parent / "outputs" / "drills_classified_sample.csv"
MODEL = "gpt-3.5-turbo"
DELAY = 1.0
...
# === FILTER TO SAMPLE ===
sample = [d for d in all_drills if d['source'] in {"IHS", "Weiss"}][:5]
print(f"🧪 Running sample classification on {len(sample)} drills")

classified = []

# === MAIN LOOP ===
for i, drill in enumerate(sample):
    print(f"\n🔍 Classifying drill {i+1}/{len(sample)}: {drill['title']}")
